# Customer Screens Overview

## Purpose

Index of UI screen specifications under **ग्राहक (Customer)**. Each linked document is self-contained for AI agents generating frontend screens.

Branch lookups use a single **Autocomplete** field — see [../shared/entity-autocomplete-pattern.md](../shared/entity-autocomplete-pattern.md).

## Screens

| Screen (Marathi) | Screen (English) | Document |
| :--- | :--- | :--- |
| ग्राहक यादी | Customer List | [customer-list-screen.md](customer-list-screen.md) |
| नवीन ग्राहक | New Customer | [new-customer-screen.md](new-customer-screen.md) |
| नवीन इतर खाते | New Other Account | [new-other-account-screen.md](new-other-account-screen.md) |
| इतर खाते नोंदणी | Other Account Registration | [other-account-registration-screen.md](other-account-registration-screen.md) |

## Screenshot Location

```text
screenshots/grahak_khatedar/
```

## Common Patterns

- **Breadcrumb root:** `डॅशबोर्ड > ग्राहक > <Screen>`
- **Required-field legend:** `(*) आवश्यक फील्ड दर्शविते`
- Customer search filters share: Organization, Branch Code, Branch, Customer Name, Customer No. range, Status

## Related Documents

- [changelog.md](changelog.md)
- [../membership/overview.md](../membership/overview.md)
- [../../AI_INDEX.md](../../AI_INDEX.md)
