...
| `02-business-domains/` (business rules, use cases) | 🟡 In progress — `@generate-business-domain` skill + **Settings/Master** template (2026-07-23); Customer next |
...
| Skill files | ✅ `generate-document`, `generate-ui-screen`, `generate-ui-mockup`, `design-database-schema`, `generate-business-domain`, `optimize-ui-ux`, `coding-standards` |
...
**Resequenced 2026-07-23:** Settings/Masters first (foundational master data gates all downstream modules), then Customer. Each Settings sub-area mirrors `05-ui-ux/settings/<sub-area>/` under `02-business-domains/settings/<sub-area>/`.

**Phase 1.7a — Settings/Master (template sub-area):**
- [x] `@generate-business-domain` skill + rule (`.cursor/skills/generate-business-domain/`, `.cursor/rules/generate-business-domain.mdc`)
- [x] `02-business-domains/settings/master/overview.md`
- [x] `business-rules.md` (BR-001…)
- [x] `use-cases.md` (UC-001…)
- [x] `workflows.md`
- [x] `acceptance-tests.md` (AT-001…)
- [x] `changelog.md`

**Phase 1.7b — Remaining Settings sub-areas** (same file set per sub-area):
- [ ] `02-business-domains/settings/accounting/` — GL Group/Head setup
- [ ] `02-business-domains/settings/schemes/` — unified New Scheme (all product types)
- [ ] `02-business-domains/settings/membership/` — membership configuration
- [ ] `02-business-domains/settings/loan/` — loan interest rate change

**Phase 1.7c — Customer** (next full domain after Settings):
- [ ] `02-business-domains/customer/` — same file set (`overview`, `business-rules`, `use-cases`, `workflows`, `acceptance-tests`, `changelog`)

> Use Settings/Master + Customer as the **template** so remaining domains (Membership, Accounts, Loans, Accounting operational) follow identical structure in Phase 3. **Settings/Master is the primary Phase 1 unblocker in progress.**
...
**Closed since last plan update:** §1.1, §1.3 (all AI Context), §1.5 (mockups), §1.6 (component catalog), technology-stack + DEC log, User Role matrix (dynamic registry), §1.7a Settings/Master business-domain template + `@generate-business-domain` skill.

**Focus next (unblocks Phase 1 exit → Phase 2):**

1. **§1.7b remaining Settings sub-areas** — `accounting/`, `schemes/`, `membership/`, `loan/` via `@generate-business-domain settings/<sub-area>`.
2. **§1.7c Customer domain** — author `02-business-domains/customer/` using the same template.
3. **§1.8 in parallel:** run `@design-database-schema shared` then `settings/master` + `customer` entity docs; draft `03-api-contracts/` for Auth/User + Customer endpoints only.
4. **§1.2 remaining architecture docs** (can run alongside §1.7): `domain-driven-design.md` + `security-architecture.md` (JWT tenant claim + RBAC levels). Needed before Phase 2 auth skeleton.
5. **Defer:** Deposit Loan Installment Tabs 2–6 capture — not on Settings/Customer or Phase 2 critical path.

**Do not start Phase 2 code** until Settings/Master + Customer business domains + Auth/Customer slice of §1.8 + `security-architecture.md` exist — otherwise the skeleton will invent contracts and entities.
